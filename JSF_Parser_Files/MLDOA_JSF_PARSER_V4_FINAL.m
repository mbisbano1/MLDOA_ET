...
% Determine Default CSV Filenames from Input JSF Filenames.
...
% Specify Output Location of CSV Files
...
OutMatNumPings = input('Enter the Number of Pings to initialize OutMat with (default 1287)');
OutMatNumSamples = input('Enter the Number of Samples Per Ping to initialize OutMat with (default 4340)');
OutMat = -69.*ones(OutMatNumPings*OutMatNumSamples, 32);
...
% User defineable parameters
    SamplesPerPing = OutMatNumSamples;
    %SamplesPerPing = 4340 ;     % 4340 samples per ping (IN 002 dataset)
    %SamplesPerPing = 6508 ;     % 6508 samples per ping (IN 038 dataset)
    
    % init 1
    PingCtr = 1 ;
    
    MaxPingCtr = OutMatNumPings;
    %MaxPingCtr = 10 ;
    %MaxPingCtr = 1287 ;
    %MaxPingCtr = 3484;
    %MaxPingCtr = 3000 ;
    ...
    % Init as impossible vals to search for correct ones later
...
% Initialize Message 80 Pings() elements for efficiency
    ...
    %   PingNum(PingIndex)      
    %   to determine if we are off by an index of one or something
    ...
    %                                           1       2
    %   StaveData(PingIndex, Channel#, Sample#, Timestamp/Data)  Channels
    %   (1,3,5,... port), (20,18,16,... stbd)
    Pings.StaveData = NaN(MaxPingCtr, NumChannelsPerSample, SamplesPerPing, 2) ; 
    % EX. t = Ping 3, Stbd Channel 1, Sample 206, Timestamp: 
    %   t = Pings.StaveData(3, 20, 206, 1) ;
    % EX. d = Ping 3, Stbd Channel 1, Sample 206, Data: 
    %   d = Pings.StaveData(3, 20, 206, 2) ;    
    Pings.SampleFreq = NaN(MaxPingCtr, 1);

% Loop through all JSF File Messages for all stave data, store in Pings
% object..
while PingCtr <= MaxPingCtr
    ...
    elseif ((mH.contentType == 1) && (mH.messageType == 80))  % Message 80!
        if mH.subsystem == 40 || mH.subsystem == 40 % stave data combining port and starboard
            ...
            % These find the min & max of ping num and samples/ping
...
% MeasurementData(PingIndex , Port/Stbd, Sample#, TimeStamp/RTT/Angle/Amplitude/AngleUncertainty/SampleRate)
    ...
    %********************************************************************
% Loop through reading every message in the Bathy data JSF file, collecting all bathy data in Measurements object.    
...
%% A plot for comparing Port and Stbd available data.
% you can probably ignore this unless you are specifically looking for
% something.
figure(1);
...
% TWTT is not recorded for each sample in the Stave data, so values of TWTT
% must be interpolated (from samplerate) and aligned with those in the
% Bathy data to ensure the inputs correspond to the right outputs!

% We create the Soundings object to act as the correctly shifted version of
% Measurements object.
...
for portPings = 1:MaxPingCtr
   % TWTTs are the two way travel times of ping num portPings, on the port
   % side, compressed to just a 1D array.
...
for stbdPings = 1:MaxPingCtr
   % TWTTs are the two way travel times of ping num stbdPings, on the stbd
   % side, compressed to just a 1D array. 
   ...
   for stbdSamps = 1:length(RNSSP)
        ...
        Soundings.Angle(stbdPings, 2, RNSSP(stbdSamps)+1) = Measurements.Angle(stbdPings, 2, stbdSamps).*(180/pi);      % now in degrees
        Soundings.Amplitude(stbdPings, 2, RNSSP(stbdSamps)+1) = Measurements.Amplitude(stbdPings, 2, stbdSamps);      
...
%% Format all this information into Port Side OutMat
% Data Formatting. This is REALLY inefficient, a better way to do this
% would be nice, maybe try replacing some for loop operations with matrix
% operations.
...
PortOutMat = OutMat;
clear OutMat
%% Format all this information into Stbd Side OutMat

% Reset OutMat since it was just cleared..
OutMat = -69.*ones(OutMatNumPings*OutMatNumSamples, 32);
    %OutMat = -69.*ones(1287*4340, 32);
    %OutMat = -69.*ones(888*6508, 32);
    %OutMat = -69.*ones(3484*6508, 32);
...
%% find all rows with missing data & remove...
...
%% Write Port Matrix out to CSV File
...
% Output Management
    ...
    disp(message);
%% Write Stbd Matrix out to CSV File
    ColumnNames = {'PingNum', 'SampleNum', 'PortStbd', 'SampleTimeDelay', ...
            'I1','Q1', 'I2', 'Q2', 'I3', 'Q3', 'I4', 'Q4',...
            'I5', 'Q5', 'I6', 'Q6', 'I7', 'Q7', 'I8', 'Q8',...
            'I9', 'Q9', 'I10', 'Q10', 'Roll', 'C', ...
...
%% Print out Acoustic and Sample Frequencies
...
disp('Expected: Acoustic Frequency should be ~540 kHz, Sample Frequency should be ~60kHz');